...
function filterAndDedup(files) {
    return files
        .filter((f) => {
        if (f.endsWith(".d.ts") || f.endsWith(".d.js"))
            return false;
        ...
        const ext = path.extname(f);
        const base = path.basename(f, ext);
        const alreadyHasTs = acc.some((x) => path.basename(x, path.extname(x)) === base && x.endsWith(".ts"));
        if (ext === ".js" && alreadyHasTs)
            return acc;
        const jsIdx = acc.findIndex((x) => path.basename(x, path.extname(x)) === base && x.endsWith(".js"));
        if (ext === ".ts" && jsIdx !== -1) {
            acc[jsIdx] = f;
            return acc;
        }
...
export function buildKnexConfig(db) {
    const migrations = {
        migrationSource,
    };
