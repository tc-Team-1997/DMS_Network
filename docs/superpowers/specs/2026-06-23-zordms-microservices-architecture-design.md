...
- **Enterprise licensing with scalability** (tender item 4).
...
|---|---------|-------|-----------------------------------------------|
| 1 | **Gateway / Identity** | Node/Express | AuthN (session + JWT), RBAC matrix (6 roles), MFA (TOTP), SSO (SAML2 / AD), API keys, rate-limiting, request routing/aggregation (BFF), centralized audit log. → *Security & Access Control*. |
...
|---|---|---|
| 1–4 | Microservices, web-based, Unicode/multi-lang, enterprise licensing | Topology + apps/web |
...
| 21–23 | Security: RBAC, MFA + SSO, AES-256 + audit | Gateway |
...
- React design-system component inventory mapped to the 19 screens.
