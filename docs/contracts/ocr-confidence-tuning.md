...
> **Per-doctype OCR confidence threshold control in Document Types admin page (Learn Wizard).** Dual-range slider for autofill_floor + confidence_high. Closes bob-compliance-summary §72 and bidding §31.
...
**Personas affected:**
- `Doc Admin` — opens Document Types > select doctype > Thresholds tab. Adjusts sliders for autofill_floor (gold) + confidence_high (green). Sees live preview of sample document with new thresholds.
...
- **AC-1** — Given a Document Type with `autofill_floor=0.4` and `confidence_high=0.7`, when the Doc Admin opens the Thresholds tab, two horizontal range sliders are visible: gold slider (autofill_floor, 0–1 scale, ticks every 0.1) and green slider (confidence_high, 0–1 scale). Current values shown as numbers.
...
- **AC-3** — Given sliders adjusted but not saved, the "Reset to defaults" button restores sliders to 0.4 (autofill_floor) and 0.7 (confidence_high). "Save" button persists via `PATCH /spa/api/document-types/{id} { autofill_floor, confidence_high }`.
- **AC-4** — Given a successful save, audit_log entry is written: action `DOCTYPE_THRESHOLD_UPDATED`, details `{ old_autofill_floor, new_autofill_floor, old_confidence_high, new_confidence_high, changed_by }`.
...
[Thresholds tab renders]
              ...
              │  ├─ confidence_high (green): "Show user extraction, request review if below"
              │
...
[Admin clicks "Save" button]
              ...
              │
              ├─ POST /spa/api/document-types/{id} { autofill_floor: 0.5, confidence_high: 0.7 }
              │
...
[Python endpoint]
              │
              ├─ validates 0 <= autofill_floor <= 1, 0 <= confidence_high <= 1
              │
              ├─ validates autofill_floor <= confidence_high (rule: low threshold ≤ high threshold)
              │
              ├─ updates document_type_schemas row
              │
              ├─ writes audit_log { action: DOCTYPE_THRESHOLD_UPDATED, details: {...} }
              │
              ├─ returns 200 { id, autofill_floor, confidence_high, updated_at }
              │
              ▼
...
| --- | --- | --- | --- | --- |
| `PATCH` | `/api/v1/document-types/{id}` | `require_api_key` + JWT(role≥doc_admin) | Y | Update doctype (extend: add autofill_floor, confidence_high, tested_with_sample_id) |
...
// PATCH — 400 (validation)
{
  ...
  "details": {
    ...
    "confidence_high": "must be >= autofill_floor"
  }
}
```
...
export const DocumentType = z.object({
  ...
  confidence_high: z.number().min(0).max(1).default(0.7),
...
export const ThresholdUpdate = z.object({
  ...
  (data) => data.autofill_floor <= data.confidence_high,
  { message: "autofill_floor must be <= confidence_high", path: ["autofill_floor"] }
);
...
3. **Green slider (confidence_high):** Label "Confidence threshold for review (0–100%)", numeric display "0.70 (70%)", track colored green.
4. Both sliders have tick marks every 0.1 (0, 0.1, 0.2, ..., 1.0) + numeric label at each tick.
5. Sliders move independently but autofill_floor cannot exceed confidence_high (UI constraint: if user drags autofill_floor above confidence_high, it snaps back; or reorder them).
...
7. **Extraction Preview section:**
   ...
   - Status color-coded: green "auto-fill" (>= autofill_floor), yellow "review" (>= confidence_high but < autofill_floor), red "skip" (< confidence_high).
...
ALTER TABLE document_type_schemas ADD COLUMN IF NOT EXISTS confidence_high REAL DEFAULT 0.7;
...
class DocumentTypeSchema(Base):
    ...
    confidence_high = Column(Float, nullable=False, default=0.7)
...
**Defaults:** autofill_floor=0.4, confidence_high=0.7 (tuned for balanced precision/recall across doc types).
...
| Audit | Every threshold change writes to `audit_log`: action `DOCTYPE_THRESHOLD_UPDATED`, details `{ doctype_id, old_autofill_floor, new_autofill_floor, old_confidence_high, new_confidence_high, changed_by, changed_at }`. User SID included. |
...
| Input validation | autofill_floor and confidence_high must be in [0, 1]. autofill_floor must be <= confidence_high (Zod refine). tested_with_sample_id must reference an existing sample in same doctype. |
...
- **Log** — structured line: `{ts, tenant_id, doctype_id, action: "DOCTYPE_THRESHOLD_UPDATED", old/new thresholds, latency_ms}`
...
- Distribution of autofill_floor and confidence_high values per tenant.
...
- **i18n** — All strings via `t()`: `t('doctype.thresholds_tab')`, `t('doctype.autofill_floor_label')`, `t('doctype.confidence_high_label')`, `t('doctype.reset_button')`, `t('doctype.save_button')`. Keys in `apps/web/src/i18n/{en,dz}.json`.
...
| Case | Trigger | UX |
| --- | --- | --- |
| Validation — autofill > confidence | User drags autofill_floor above confidence_high | Slider snaps back to confidence_high value. Toast: "Autofill floor cannot exceed confidence high." |
...
| --- | --- | --- | --- |
| Unit (Zod) | `apps/web/src/modules/document-types/schemas.ts` | `spa-engineer` | ThresholdUpdate refine: autofill_floor <= confidence_high |
...
- **Impact on extraction quality** — Track before/after: documents requiring manual extraction review (confidence between autofill_floor and confidence_high) should decrease by 20% on average after tenants tune thresholds to their data distribution.
...
- [ ] audit_log entries land for every threshold change (manual smoke: adjust thresholds, verify DOCTYPE_THRESHOLD_UPDATED in logs with old/new values)
