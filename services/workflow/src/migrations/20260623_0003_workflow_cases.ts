...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable("workflows", (t) => {
    ...
    t.string("doc_id", 80);
    t.string("template_id", 36).references("id").inTable("workflow_templates").onDelete("SET NULL");
  ...
  await knex.schema.createTable("workflow_steps", (t) => {
    t.string("id", 36).notNullable().primary();
    t.string("workflow_id", 36).notNullable().references("id").inTable("workflows").onDelete("CASCADE");
  ...
  await knex.schema.createTable("cases", (t) => {
    ...
    t.string("assigned_to", 100);
    t.timestamp("due_at");
    t.string("workflow_id", 36).references("id").inTable("workflows").onDelete("SET NULL");
  ...
  await knex.schema.createTable("case_documents", (t) => {
    t.string("id", 36).notNullable().primary();
    t.string("case_id", 36).notNullable().references("id").inTable("cases").onDelete("CASCADE");
