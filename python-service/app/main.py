...
# aml_screening_router is registered before the legacy aml_router so that
# new /api/v1/aml/* routes take precedence when paths overlap (FastAPI
...
app.include_router(worm_router.router)

# Document redaction — BHU-46 (PDF text destruction, region-based, irreversible)
from .routers import document_redaction as document_redaction_router  # noqa: E402
app.include_router(document_redaction_router.router)
...
from .routers import ai_glossary as ai_glossary_router  # noqa: E402
...
async def _startup():
    ...
    # Best-effort DMS knowledge ingest — populates the vector store so the
    # AI Engine agent's search_knowledge tool can answer narrative questions
    # even on a fresh install. Non-fatal: if Ollama is down we just log and
    # continue; the search() call will lazily retry.
    try:
        from .services.docbrain import knowledge as dms_knowledge
        dms_knowledge.ingest()
